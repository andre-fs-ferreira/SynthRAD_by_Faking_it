...
class Upsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, use_trilinear_up, sample_kernel, dims=2, out_channels=None):
        super().__init__()
        self.channels = channels
        self.out_channels = out_channels
        self.use_conv = use_conv
        self.use_trilinear_up = use_trilinear_up
        if dims == 3:
        ...
        elif use_trilinear_up:
            self.up = th.nn.Upsample(scale_factor=self.sample_kernel,mode='trilinear')
            self.conv = zero_module(conv_nd(dims, self.channels, self.out_channels, 1, padding=0)) # Light!
        else:
            self.up = th.nn.Upsample(scale_factor=self.sample_kernel,mode='nearest')
    ...
    def forward(self, x):
        ...
        if self.use_conv or self.use_trilinear_up:
             x = self.conv(x)
        return x
...
class Downsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, use_avg_pool_down, sample_kernel, dims=2, out_channels=None):
        super().__init__()
        self.channels = channels
        ...
        elif use_avg_pool_down:
            self.op = avg_pool_nd(dims=dims, kernel_size=sample_kernel, stride=sample_kernel)
        else:
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        conv_resample=False,
        use_avg_pool_down=False,
        use_trilinear_up=False,
        dims=2,
        sample_kernel = None,
        num_classes=None,
        use_checkpoint=False,
        ...
        self.use_avg_pool_down = use_avg_pool_down
        self.use_trilinear_up = use_trilinear_up
                ...
                self.input_blocks.append(
                    TimestepEmbedSequential(
                        ...
                        if resblock_updown
                            ...
                            ch, conv_resample, use_avg_pool_down, self.sample_kernel[level], dims=dims, out_channels=out_ch
                        )
                    )
                )
                ch = out_ch
            ...
            for i in range(num_res_blocks+1):
                ...
                if level and i == num_res_blocks:
                    ...
                    layers.append(
                        ...
                        if resblock_updown
                        else Upsample(ch, conv_resample, use_trilinear_up, self.sample_kernel[level-1], dims=dims, out_channels=out_ch)
                    )
                    ds *= 2
