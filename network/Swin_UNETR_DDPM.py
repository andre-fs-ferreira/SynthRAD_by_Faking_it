...
# Adaptation to DDPM by André Ferreira based on https://github.com/YutaZhuo/DiffuseReg
...
from _monai import UnetrBasicBlock, UnetrUpBlock
...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        self.res_time_dim = feature_size  # = feature_size or None
        self.depths = depths
        ...
        self.encoder1 = UnetrBasicBlock(
            ...
            norm_name=norm_name,
            res_block=True,
            time_emb_dim=self.res_time_dim
        )
        ...
        # add a time_linear for each resolution
        for i in range(len(self.depths)):
            layer = nn.Sequential(
                nn.SiLU(),
                nn.Linear(self.embed_dim, self.embed_dim * (2 ** i)))
            layer_name = f'time_linear{i}'
    ...
    def forward(self, x_in, t):
        ...
        enc0 = self.encoder1(x_in, time_token)
        enc1 = self.encoder2(hidden_states_out[0], time_token)
        enc2 = self.encoder3(hidden_states_out[1], time_token)
        enc3 = self.encoder4(hidden_states_out[2], time_token)
        dec4 = self.encoder10(hidden_states_out[4], time_token)
        dec3 = self.decoder5(dec4, hidden_states_out[3], time_token)
        dec2 = self.decoder4(dec3, enc3, time_token)
        dec1 = self.decoder3(dec2, enc2, time_token)
        dec0 = self.decoder2(dec1, enc1, time_token)
        out = self.decoder1(dec0, enc0, time_token)
        logits = self.out(out)
        return logits
